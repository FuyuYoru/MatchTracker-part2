...
import { TeamTitle } from "@/features/matches/ui/MatchCard/TeamTitle";
import { MatchStatus } from "@/features/matches/ui/MatchCard/MatchStatus";
import { TeamStatistics } from "@/features/matches/ui/MatchCard/TeamStatistics";
import { PlayerStatistics } from "@/features/matches/ui/MatchCard/PlayerStatistics";
import { Player } from "@/features/matches/model";
...
export const MatchCard: React.FC<{ matchData: any }> = ({ matchData }) => {
  ...
  return (
    <View className="flex flex-col mb-[12] bg-[#0B0E12] p-[8] sm:px-4 rounded-[4px] w-full gap-[8]">
      <Pressable onPress={handlePress} className="gap-[8]">
          ...
          <View className="flex sm:flex-row sm:gap-[32]">
            <View className="flex flex-col flex-1 gap-[8]">
              <ScrollView contentContainerClassName="flex flex-row gap-[8]">
                {matchData.homeTeam.players.map(
                  (item: Player, index: number) => (
            ...
            <View className="flex flex-col flex-1  gap-[8]">
              <ScrollView contentContainerClassName="flex flex-row gap-[8]">
                {matchData.awayTeam.players.map(
                  (item: Player, index: number) => (
                    <PlayerStatistics player={item} key={index} />
                  )
                )}
